...
| 3 | No critical/high unresolved security or child-safety finding remains | **Done** | `npm audit` originally found 2 real high-severity advisories in `next@14.2.35`; fixed by upgrading to `next@16.3.3` (a real major-version migration, code changes and all — see `00-hardening-report.md`'s update note). `npm audit --omit=dev` now reports zero vulnerabilities across the monorepo. No child-safety-specific finding is known. |
